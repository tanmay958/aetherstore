"""Search a REES46 CSV using the in-memory index.

Builds the whole index in RAM and queries it, which is exactly the thing the
segment format exists to avoid. Useful as a demonstration of the data
structure and as a sanity check on real data, not as anything to run at scale.
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"index: {index.num_docs:,} docs, {index.num_terms:,} terms, "
        f"{index.num_postings:,} postings   built in {build_ms:,.1f} ms"
    )
